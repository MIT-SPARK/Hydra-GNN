...
def convert_to_junction_tree_hierarchy(data: Data, task='graph', node_id=None, radius=None) -> Data:
    """
...
def convert_to_same_junction_tree_hierarchy(data: Data, min_diameter=None, max_diameter=None):
    """
        ...
        for data in dataset:
            if radius is None:  # for nodes in the same graph, use the same junction tree hierarchy
                train_graphs, val_graphs, test_graphs \
                    = convert_to_same_junction_tree_hierarchy(data, min_diameter, max_diameter)
                ...
                for i in range(data.num_nodes):
                    if 100.0 * i / data.num_nodes > progress_threshold + 1:
                        ...
                        print("Progress converting ego graphs to JTH: ", progress_threshold, "/", "100 %")
                    if data.train_mask[i].item() or data.val_mask[i].item() or data.test_mask[i].item():
                        data_jth = convert_to_junction_tree_hierarchy(data, task, node_id=i, radius=radius)
                        if (min_diameter is None or data_jth.diameter >= min_diameter) and \
                                (max_diameter is None or data_jth.diameter <= max_diameter):
...
def get_subtrees_from_jth(data, G_jth, radius):
    ...
    for j in range(num_original_nodes):
        ...
        if data_mask[original_idx].item() is True:
            ...
            G_subtree = nx.ego_graph(G_jth, leaf_nodes[0], radius=radius, undirected=False)
            for leaf_node in leaf_nodes[1:]:  # add other subtrees if there are multiple leaf nodes
                if G_subtree.number_of_nodes() == G_jth.number_of_nodes():
                    break
                H_subtree = nx.ego_graph(G_jth, leaf_node, radius=radius, undirected=False)
