"""Test dataset construction for MP3D."""
...
def test_Hydra_mp3d_data(test_data_dir):
    ...
    data.add_object_edges(threshold_near=2.0, max_on=0.2, max_near=2.0)
    assert data.get_room_object_dsg().get_layer(dsg.DsgLayers.OBJECTS).num_edges() > 0
    ...
    data.compute_torch_data(
        ...
        node_converter=dsg_node_converter(
            object_feature_converter, lambda i: np.empty(300)
        ),
    ...
    # check y is computed
    assert (
        torch.numel(torch_data["objects"].y)
        == G_ro.get_layer(dsg.DsgLayers.OBJECTS).num_nodes()
    )
    assert (
        torch.numel(torch_data["rooms"].y)
        == G_ro.get_layer(dsg.DsgLayers.ROOMS).num_nodes()
    )
...
def test_fill_missing_edge_index():
    """Test room edge logic."""
    # data with 1 room
    data = HeteroData()
    data["rooms"].x = torch.rand((1, 6))
    data["objects"].x = torch.rand((3, 306))
    object_edge = torch.tensor([[0, 1, 1, 2, 2, 0], [1, 0, 2, 1, 0, 2]])
    room_object_edge = torch.tensor([[0, 0, 0], [0, 1, 2]])
    ...
    assert torch.all(
        data[("objects", "objects_to_rooms", "rooms")].edge_index
        == data["rooms", "rooms_to_objects", "objects"].edge_index.flip([0])
    ), data[("object", "objects_to_rooms", "rooms")].edge_index

    # data with 2 room
    data = HeteroData()
    data["rooms"].x = torch.rand((2, 6))
    data["objects"].x = torch.rand((4, 306))
    room_edge = torch.tensor([[0, 1], [1, 0]])
    object_edge = torch.tensor([[0, 1, 1, 2, 2, 0], [1, 0, 2, 1, 0, 2]])
    room_object_edge = torch.tensor([[0, 0, 0, 1], [0, 1, 2, 3]])
...
def test_Hydra_mp3d_dataset(test_data_dir):
    """Test resultant pytorch geometric dataset."""
    ...
    if not (
        os.path.exists(test_json_file1)
        and os.path.exists(test_json_file2)
    ...
    # sample data
    data1 = Hydra_mp3d_data(
        scene_id="x8F5xyUWy9e",
        trajectory_id=0,
        num_frames=1447,
        file_path=str(test_json_file1),
    )
    data1_copy = Hydra_mp3d_data(
        scene_id="x8F5xyUWy9e",
        trajectory_id=1,
        num_frames=1447,
        file_path=str(test_json_file1),
    )
    data2 = Hydra_mp3d_data(
    ...
    if pytest.colormap_data is None or pytest.word2vec_model is None:
        ...
        data1.add_object_edges(threshold_near=2.0, max_on=0.2, max_near=2.0)
        data1.compute_torch_data(
            use_heterogeneous=True,
            node_converter=dsg_node_converter(
                object_feature_converter, lambda i: np.empty(300)
            ),
        ...
        data2.add_object_edges(threshold_near=2.0, max_on=0.2, max_near=2.0)
        data2.compute_torch_data(
            use_heterogeneous=True,
            node_converter=dsg_node_converter(
                object_feature_converter, lambda i: np.empty(300)
            ),
