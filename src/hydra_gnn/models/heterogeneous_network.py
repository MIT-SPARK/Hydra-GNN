        ...
        **kwargs
        ...
        if output_dim is not None:
            ...
            output_dim_dict = {
                "rooms": output_dim,
                "objects": output_dim,
            }  # final objects states will be ignored
        else:
        ...
        if self.conv_block == "GAT":
            ...
            self.convs.append(
                build_hetero_conv(
                    conv_block, EDGE_TYPES, input_dim_dict, hidden_dim_dict
                )
            )
            for _ in range(1, self.num_layers - 1):
                self.convs.append(
                    build_hetero_conv(
                        conv_block, EDGE_TYPES, hidden_dim_dict, hidden_dim_dict
                    )
                )
            self.convs.append(
                build_hetero_conv(
                    conv_block, EDGE_TYPES, hidden_dim_dict, output_dim_dict
                )
            )
