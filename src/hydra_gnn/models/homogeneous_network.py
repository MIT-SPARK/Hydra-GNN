...
class HomogeneousNetwork(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.conv_block == "GAT":
            self.convs = build_GAT_conv_layers(
                input_dim,
                GAT_hidden_dims_with_last_layer,
                GAT_heads,
                GAT_concats,
                dropout=dropout,
            )
            ...
            for _ in range(1, self.num_layers - 1):
                self.convs.append(
                    build_conv_layer(self.conv_block, hidden_dim, hidden_dim)
                )
            self.convs.append(
                build_conv_layer(self.conv_block, hidden_dim, mp_output_dim)
            )
        ...
        if self.classification_task == "all":
            if self.conv_block[:3] != "GAT":
                ...
                final_hidden_dim = (
                    GAT_hidden_dims_with_last_layer[-1] * GAT_heads[-1]
                    if GAT_concats[-1]
                    else GAT_hidden_dims_with_last_layer[-1]
                )
            # remove 's' for HomogeneousNeuralTreeNetwork child class
            num_room_labels = (
                output_dim_dict["rooms"]
                if "rooms" in output_dim_dict
                else output_dim_dict["room"]
            )
            num_object_labels = (
                output_dim_dict["objects"]
                if "objects" in output_dim_dict
                else output_dim_dict["object"]
            )
    ...
    def forward(self, data):
        ...
        if self.classification_task == "room":
            ...
            x = F.relu(x) if self.conv_block[:3] != "GAT" else F.elu(x)
            x = F.dropout(x, p=self.dropout, training=self.training)
            return self.post_mp_room(x[room_mask, :]), self.post_mp_object(
                x[~room_mask, :]
            )
