...
def build_hetero_conv(
    conv_block, edge_types, input_dim_dict, output_dim_dict, aggr="sum"
):
    conv_dict = dict()
...
def build_GAT_hetero_conv(
    ...
    for i in range(len(GAT_heads)):
        convs.append(
            HeteroConv(
                {
                    edge_type: conv_module_list_dict[edge_type][i]
                    for edge_type in conv_module_list_dict.keys()
                },
                aggr=aggr,
            )
        )
...
def cross_entropy_loss(pred, label, mask=None):
    if isinstance(pred, torch.Tensor):
        ...
        if mask is None:
            loss_sum = sum(
                F.cross_entropy(p, l, reduction="sum") for p, l in zip(pred, label)
            )
            num_labels = sum(torch.numel(l) for l in label)
        else:
            loss_sum = sum(
                F.cross_entropy(p[m, :], l[m], reduction="sum")
