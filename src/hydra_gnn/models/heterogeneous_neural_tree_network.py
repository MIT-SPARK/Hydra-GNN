from hydra_gnn.models.utils import (
    build_hetero_conv,
...
class HeterogeneousNeuralTreeNetwork(nn.Module):
        ...
        **kwargs
        ...
        if disable_initialization:
            ...
            for source, edge_name, target in HTREE_INIT_EDGE_TYPES:
                conv_dict[source, edge_name, target] = pyg_nn.GATConv(
                    (input_dim_dict[source], input_dim_dict[target]),
                    input_dim_dict[target],
                    heads=1,
                    concat=False,
                    dropout=0.0,
        ...
        # message passing
        mp_input_dim_dict = {
            node_type: input_dim_dict[node_type] for node_type in HTREE_NODE_TYPES
        }
        hidden_dim_dict = {node_type: hidden_dim for node_type in HTREE_NODE_TYPES}
        if self.conv_block == "GAT":
            self.convs = build_GAT_hetero_conv(
                HTREE_EDGE_TYPES,
                mp_input_dim_dict,
                output_dim_dict,
                GAT_hidden_dims,
            ...
            self.convs.append(
                build_hetero_conv(
                    conv_block, HTREE_EDGE_TYPES, mp_input_dim_dict, hidden_dim_dict
                )
            )
            for _ in range(1, self.num_layers - 1):
                self.convs.append(
                    build_hetero_conv(
                        conv_block, HTREE_EDGE_TYPES, hidden_dim_dict, hidden_dim_dict
                    )
                )
            self.convs.append(
                build_hetero_conv(
                    conv_block, HTREE_EDGE_TYPES, hidden_dim_dict, output_dim_dict
                )
            )
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers):
            ...
            if (
                i != self.num_layers - 1
            ):  # activation and dropout, except for the last iteration
                x_dict = (
        ...
        if self.classification_task == "room":
            x_room = self.post_mp(
                x_dict["room"], edge_index_dict["room", "r_to_rv", "room_virtual"]
            ...
            x_object = self.post_mp(
                x_dict["object"], edge_index_dict["object", "o_to_ov", "object_virtual"]
