...
def get_room_object_counts(datalist):
    ...
    for mp3d_data in datalist:
        ...
        for r_index, o_index in (
            torch_data[("rooms", "rooms_to_objects", "objects")].edge_index.t().tolist()
        ):
...
def get_object_per_room(datalist):
    ...
    for mp3d_data in datalist:
        torch_data = mp3d_data.get_torch_data()
        room_degrees = pyg_utils.degree(
            torch_data["objects", "objects_to_rooms", "rooms"].edge_index[1, :],
            num_nodes=torch_data["rooms"].num_nodes,
            dtype=int,
        )
...
def get_dataset_stat(data_list):
    ...
    for data in data_list:
        ...
        if trajectory_id not in dataset_stat[scene_id]:
            dataset_stat[scene_id][trajectory_id] = {
                "num_rooms": [],
                "num_objects": [],
                "num_room_edges": [],
                "num_object_edges": [],
        ...
        dataset_stat[scene_id][trajectory_id]["num_rooms"].append(
            data.get_torch_data()["rooms"].num_nodes
        )
        dataset_stat[scene_id][trajectory_id]["num_objects"].append(
            data.get_torch_data()["objects"].num_nodes
        )
        dataset_stat[scene_id][trajectory_id]["num_room_edges"].append(
            data.get_torch_data()["rooms", "rooms_to_rooms", "rooms"].num_edges
        )
        dataset_stat[scene_id][trajectory_id]["num_object_edges"].append(
            data.get_torch_data()["objects", "objects_to_objects", "objects"].num_edges
        )
    return dataset_stat


# helper function to get maximum data stat (over frames) for each trajectory
def get_max_data_stat(
    dataset_stat,
    columns=["max_rooms", "max_objects", "max_room_edges", "max_object_edges"],
    column_key=["num_rooms", "num_objects", "num_room_edges", "num_object_edges"],
    func=max,
):
    ...
    for scene_id in dataset_stat:
        for trajectory_id in dataset_stat[scene_id]:
            ...
            df.loc[len(df.index)] = [f"{scene_id}_{trajectory_id}"] + [
                func(stat_dict[key]) for key in column_key
            ]
...
def plot_room_object_heatmap(
    room_object_data,
    object_label_list=object_label_list,
    room_label_list=room_label_list,
    title="Room-object Heatmap",
):
    fig = go.Figure()
    fig.add_trace(
        go.Heatmap(
...
def plot_room_room_heatmap(
    room_room_data, room_label_list=room_label_list, title="Room-Room Heatmap"
):
    fig = go.Figure()
    fig.add_trace(
        go.Heatmap(
...
def plot_object_barchart(
    data_dict, object_label_list=object_label_list, title="Object occurrences"
):
    fig = go.Figure()
    for data_name, data in data_dict.items():
        fig.add_trace(
            go.Bar(name=data_name, y=object_label_list, x=data, orientation="h")
        )

    fig.update_layout(barmode="group", height=len(object_label_list) * 30, title=title)
    return fig


def plot_room_barchart(
    data_dict, room_label_list=room_label_list, title="Room occurrences"
):
    fig = go.Figure()
    for data_name, data in data_dict.items():
        fig.add_trace(
            go.Bar(name=data_name, x=room_label_list, y=data, orientation="v")
        )
    fig.update_layout(barmode="group", height=len(object_label_list) * 30, title=title)
    return fig
