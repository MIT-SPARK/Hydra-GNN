    ...
    def __init__(self, dataset, network_params, double_precision=False):
        ...
        if self._network_type == 'baseline':
            self._update_training_params(
                network_params={'output_dim_dict': 
            ...
            for batch in data_loader:
                ...
                if self._graph_type == 'homogeneous':
                    if self._network_type == 'baseline':
                        ...
                        mask = batch.train_mask[batch.room_mask], batch.train_mask[~(batch.room_mask)]
                    else:
                        label = batch.y[batch.room_mask], batch.y[batch.object_mask]
                        mask = batch.train_mask[batch.room_mask], batch.train_mask[batch.object_mask]
                else:
                    if self._network_type == 'baseline':
                        ...
                        mask = batch['room_virtual'].train_mask, batch['object_virtual'].train_mask
        ...
        for batch in data_loader:
            with torch.no_grad():
                ...
                if self._graph_type == 'homogeneous':
                    if self._network_type == 'baseline':
                        label = batch.y[batch.room_mask], batch.y[~(batch.room_mask)]
                        mask = batch[mask_name][batch.room_mask], batch[mask_name][~(batch.room_mask)]
                    else:
                        label = batch.y[batch.room_mask], batch.y[batch.object_mask]
                        mask = batch[mask_name][batch.room_mask], batch[mask_name][batch.object_mask]
                else:
                    if self._network_type == 'baseline':
                        label = batch['rooms'].y, batch['objects'].y
