from hydra_gnn.mp3d_dataset import (
    Hydra_mp3d_data,
    EDGE_TYPES,
    heterogeneous_data_to_homogeneous,
...
STANFORD3D_ROOM_SEMANTIC_LABEL_DICT = dict(
    zip(room_semantic_labels, range(num_room_labels))
)
STANFORD3D_OBJECT_SEMANTIC_LABEL_DICT = dict(
    zip(object_semantic_labels, range(num_object_labels))
)
STANFORD3D_ROOM_SEMANTIC_LABEL_DICT[
    "pantry_room"
] = STANFORD3D_ROOM_SEMANTIC_LABEL_DICT["kitchen"]
STANFORD3D_ROOM_SEMANTIC_LABEL_DICT[
    "television_room"
] = STANFORD3D_ROOM_SEMANTIC_LABEL_DICT["living_room"]
STANFORD3D_ROOM_SEMANTIC_LABEL_DICT[
    "childs_room"
] = STANFORD3D_ROOM_SEMANTIC_LABEL_DICT["playroom"]


def Stanford3DSG_object_feature_converter(
    word2vec_model, object_semantic_dict=STANFORD3D_OBJECT_SEMANTIC_LABEL_DICT
):
    """
    ...
    return lambda i: np.mean(
        [
            word2vec_model[s]
            for s in next(
                label for label, idx in object_semantic_dict.items() if idx == i
            ).split(" ")
        ],
        axis=0,
    )
...
def Stanford3DSG_room_feature_converter(
    word2vec_model, room_label_dict=STANFORD3D_OBJECT_SEMANTIC_LABEL_DICT
):
    """
    Returns a function that takes in label index and returns word2vec semantic embedding vector
    ...
    return lambda i: np.mean(
        [
            word2vec_model[s]
            for s in next(
                label for label, idx in room_label_dict.items() if idx == i
            ).split("_")
        ],
        axis=0,
    )
...
class Stanford3DSG_data(Hydra_mp3d_data):
    ...
    def __init__(
        ...
        if file_path is not None:
            ...
            self._G_ro = self.get_room_object_dsg_from_gt(
                graph_data,
    ...
    @staticmethod
    def get_room_object_dsg_from_gt(
        graph_data,
        object_semantic_dict=STANFORD3D_OBJECT_SEMANTIC_LABEL_DICT,
        room_semantic_dict=STANFORD3D_ROOM_SEMANTIC_LABEL_DICT,
    ):
        ...
        for key, object_data in graph_data["object"].items():
            ...
            if object_data["class_"] not in object_semantic_dict.keys():
                continue
            if (
                graph_data["room"][object_data["parent_room"]]["scene_category"]
                not in room_semantic_dict.keys()
            ):
                continue
            ...
            object_attr.bounding_box = dsg._dsg_bindings.BoundingBox(
            ...
            if not G_ro.has_node(room_id.value):
                ...
                room_attr.semantic_label = room_semantic_dict[
                    room_data["scene_category"]
                ]
                room_attr.bounding_box = dsg._dsg_bindings.BoundingBox(
    ...
    def get_room_object_dsg_from_preprocessed_data(data_dict):
        ...
        room_attr.bounding_box = dsg._dsg_bindings.BoundingBox(
            room_attr.position - x_room[3:] / 2, room_attr.position + x_room[3:] / 2
        )
        room_attr.bounding_box.world_P_center = room_attr.position
        G_ro.add_node(dsg.DsgLayers.ROOMS, room_id.value, room_attr)
    ...
    def compute_torch_data(
        self, use_heterogeneous: bool, node_converter, double_precision=False
    ):
        ...
        if use_heterogeneous:  # for HeteroData, make sure all edges are there
            ...
            assert (
                self._torch_data["rooms"].num_node_features
                == self._torch_data["objects"].num_node_features
            )
        else:
    ...
    def to_homogeneous(
        self, removed_edge_type=("rooms", "rooms_to_objects", "objects")
    ):
        if isinstance(self._torch_data, HeteroData):
            self._torch_data, node_types = heterogeneous_data_to_homogeneous(
                self._torch_data, removed_edge_type=removed_edge_type
            )
    ...
    def get_label_dict(self):
        return {
            "objects": self._object_semantic_dict,
...
class Stanford3DSG_htree_data(Stanford3DSG_data):
    ...
    def compute_torch_data(
        ...
        Stanford3DSG_data.compute_torch_data(
            self,
        ...
        self._torch_data["object_virtual"].y = self._torch_data[
            "object_virtual"
        ].label.long()
        self._torch_data["room_virtual"].y = self._torch_data[
            "room_virtual"
        ].label.long()
...
class Stanford3DSG_dataset(torch.utils.data.Dataset):
    ...
    def generate_node_split(
        self, train_node_ratio, val_node_ratio, test_node_ratio, seed=0
    ):
        random.seed(seed)
        ...
        if train_node_ratio + val_node_ratio + test_node_ratio == 1:
            ...
            random_assignment_list = (
                [1] * num_train_nodes + [2] * num_val_nodes + [3] * num_test_nodes
            )
        else:
            num_test_nodes = round(total_num_nodes * test_node_ratio)
            num_unassigned_nodes = (
                total_num_nodes - num_train_nodes - num_val_nodes - num_test_nodes
            )
            random_assignment_list = (
                [1] * num_train_nodes
                + [2] * num_val_nodes
                + [3] * num_test_nodes
                + [0] * num_unassigned_nodes
            )
        ...
        for data in self._data_list:
            ...
            node_split = torch.tensor(
                random_assignment_list[:num_graph_nodes], dtype=torch.int
            )
            ...
            if data_type == "homogeneous":
                ...
                data.get_torch_data()["objects"].train_mask = (
                    node_split[:num_objects] == 1
                )
                data.get_torch_data()["rooms"].train_mask = (
                    node_split[num_objects:] == 1
                )
                data.get_torch_data()["objects"].val_mask = (
                    node_split[:num_objects] == 2
                )
                data.get_torch_data()["rooms"].val_mask = node_split[num_objects:] == 2
                data.get_torch_data()["objects"].test_mask = (
                    node_split[:num_objects] == 3
                )
                ...
                data.get_torch_data().train_mask = torch.zeros(
                    data.get_torch_data().num_nodes, dtype=torch.bool
                )
                data.get_torch_data().train_mask[object_mask] = (node_split == 1)[
                    :num_objects
                ]
                data.get_torch_data().train_mask[room_mask] = (node_split == 1)[
                    num_objects:
                ]
                data.get_torch_data().val_mask = torch.zeros(
                    data.get_torch_data().num_nodes, dtype=torch.bool
                )
                data.get_torch_data().val_mask[object_mask] = (node_split == 2)[
                    :num_objects
                ]
                data.get_torch_data().val_mask[room_mask] = (node_split == 2)[
                    num_objects:
                ]
                data.get_torch_data().test_mask = torch.zeros(
                    data.get_torch_data().num_nodes, dtype=torch.bool
                )
                data.get_torch_data().test_mask[object_mask] = (node_split == 3)[
                    :num_objects
                ]
                data.get_torch_data().test_mask[room_mask] = (node_split == 3)[
                    num_objects:
                ]
            elif data_type == "heterogeneous_htree":
                num_objects = data.get_torch_data()["object_virtual"].num_nodes
                data.get_torch_data()["object_virtual"].train_mask = (
                    node_split[:num_objects] == 1
                )
                data.get_torch_data()["room_virtual"].train_mask = (
                    node_split[num_objects:] == 1
                )
                data.get_torch_data()["object_virtual"].val_mask = (
                    node_split[:num_objects] == 2
                )
                data.get_torch_data()["room_virtual"].val_mask = (
                    node_split[num_objects:] == 2
                )
                data.get_torch_data()["object_virtual"].test_mask = (
                    node_split[:num_objects] == 3
                )
                data.get_torch_data()["room_virtual"].test_mask = (
                    node_split[num_objects:] == 3
                )
            else:
    ...
    def print_data_split(self):
        ...
        if data_type == "homogeneous":
            count_func = (
                lambda split_mask, is_room: sum(
                    torch_data[split_mask][torch_data["room_mask"]].sum().item()
                    for torch_data in self
                )
                if is_room
                else sum(
                    torch_data[split_mask][~torch_data["room_mask"]].sum().item()
            ...
            count_func = (
                lambda split_mask, is_room: sum(
                    torch_data["rooms"][split_mask].sum().item() for torch_data in self
                )
                if is_room
                else sum(
                    torch_data["objects"][split_mask].sum().item()
                    for torch_data in self
                )
            )
        elif data_type == "homogeneous_htree":
            count_func = (
                ...
                if is_room
                    ...
                    torch_data[split_mask][torch_data.object_mask].sum().item()
                    for torch_data in self
                )
            )
        else:
            count_func = (
                lambda split_mask, is_room: sum(
                    torch_data["room_virtual"][split_mask].sum().item()
                    for torch_data in self
                )
                if is_room
                else sum(
                    torch_data["object_virtual"][split_mask].sum().item()
                    for torch_data in self
                )
            )

        train_room, val_room, test_room = (
            count_func("train_mask", True),
            count_func("val_mask", True),
            count_func("test_mask", True),
        )
        train_object, val_object, test_object = (
            count_func("train_mask", False),
            count_func("val_mask", False),
            count_func("test_mask", False),
        )
        print(
            f"room split({train_room+val_room+test_room}): {train_room}/{val_room}/{test_room} | "
