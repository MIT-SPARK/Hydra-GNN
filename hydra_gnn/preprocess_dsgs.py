...
def add_object_connectivity(
    G, threshold_near=2.0, max_near=2.0, max_on=0.2, min_above=1.0
):
    """
...
def get_room_object_dsg(G, verbose=False):
    ...
    for object_node in G.get_layer(dsg.DsgLayers.OBJECTS).nodes:
        ...
        if parent_place.has_parent():
            ...
            G_room_object.insert_edge(object_node.id.value, parent_place.get_parent())
            if verbose:
        ...
        neighboring_dist = [
            (_dist(G, parent_place.id.value, n), n)
            for n in parent_place.siblings()
            if G.get_node(n).has_parent()
        ]
    ...
    if verbose:
        print(
            f"Found {len(invalid_objects)} invalid objects. "
...
def convert_label_to_y(
    ...
    if isinstance(torch_data, Data):
        ...
        object_y = [
            object_label_dict[l]
            for l in torch_data.label[torch_data.node_masks[2]].tolist()
        ]
        room_y = [
            room_label_dict[chr(l)]
            for l in torch_data.label[torch_data.node_masks[4]].tolist()
        ]

        torch_data.y = torch.zeros(torch_data.label.shape, dtype=torch.int32)
        torch_data.y[torch_data.node_masks[2]] = torch.tensor(
            object_y, dtype=torch.int32
        )
...
def hydra_object_feature_converter(hydra_colormap_data, word2vec_model):
    ...
    return lambda i: np.mean(
        [
            word2vec_model[s]
            for s in hydra_colormap_data["name"][i].split("_")
            if s != "of"
        ],
        axis=0,
    )
...
def dsg_node_converter(
    object_feature_converter, room_feature_converter=lambda i: np.zeros(300)
):
    """
        ...
        if x.layer == 2:  # object
            return np.hstack(
                (
                    ...
                    object_feature_converter(x.attributes.semantic_label).astype(
                        x.attributes.position.dtype
                    ),
                )
            )
        elif x.layer == 4:  # room
            return np.hstack(
                (
                    x.attributes.position,
                    x.attributes.bounding_box.max - x.attributes.bounding_box.min,
                    room_feature_converter(x.attributes.semantic_label).astype(
                        x.attributes.position.dtype
                    ),
                )
            )
